...
# Data Loading
raw_data1 = pd.read_csv("dataset/drying_actuator1.csv")
raw_data2 = pd.read_csv("dataset/drying_actuator2.csv")
...
#module.draw_plot(dat_scaled,pcscore,'autoencoder','ind', groups)

print('=====Number of Cluster=====')
# No of Cluster TEST
kmeans_list, birch_list, gmm_list = module.cluster_number_test(dat_scaled)
nocluster = pd.DataFrame({'Number': [2,3,4,5,6,7],
                          'kmeans':kmeans_list,
                          'birch': birch_list,
                          'gmm':gmm_list})
print(nocluster)
print('=====Compare Cluster=====')
...
dict, time_dict = module.cluster_compare_test(dat_scaled, params)
silhouette_list = list(dict.values())
time_list = list(time_dict.values())
compcluster = pd.DataFrame(index=['Kmeans', 'BIRCH', 'GMM', 'DBSCAN', 'MST'], columns=['Silhouette', 'Time'])
for i in range(len(dict)):
    compcluster.iloc[i,0] = silhouette_list[i]
    compcluster.iloc[i,1] = time_list[i]
print(compcluster)
# Conclusion Result